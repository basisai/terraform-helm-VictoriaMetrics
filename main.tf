locals {
  remote_write_api_url = var.remote_write_api_url_override != null && var.remote_write_api_url_override != "" ? (
    ...
    var.vm_insert_enabled ? (
      "http://${helm_release.vm.metadata[0].name}-vminsert.${helm_release.vm.metadata[0].namespace}.svc.cluster.local:${var.vm_insert_service_port}/insert/0/prometheus"
    ) : ""
  )
  ...
  remote_read_api_url = var.remote_read_api_url_override != null && var.remote_read_api_url_override != "" ? (
    ...
    var.vm_select_enabled ? (
      "http://${helm_release.vm.metadata[0].name}-vmselect.${helm_release.vm.metadata[0].namespace}.svc.cluster.local:${var.vm_select_service_port}/select/0/prometheus"
    ) : ""
  )
